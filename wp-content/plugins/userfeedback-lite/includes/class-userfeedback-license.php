...
final class UserFeedback_License {

	private static $license_option_name         = 'userfeedback_license';
	private static $network_license_option_name = 'userfeedback_network_license';

	private $site                  = array();
	private $network               = array();
	private $licensed              = false;
	private $using_network_license = false;
	...
	public function get_site_license( $force = false ) {
		if ( ! empty( $this->site ) && ! $force ) {
			...
			$site       = get_option( self::$license_option_name, array() );
			$this->site = $site;
			return $site;
		}
	}
	...
	public function get_network_license( $force = false ) {
		if ( ! empty( $this->network ) && ! $force ) {
			...
			$network       = get_site_option( self::$network_license_option_name, array() );
	...
	private function license_to_use() {
		if ( is_network_admin() ) {
			...
			if ( ! empty( $license ) ) {
				$this->licensed              = true;
				$this->using_network_license = true;
			}
		} else {
			$license = $this->get_site_license_key();
			if ( ! empty( $license ) ) {
				$this->licensed              = true;
				$this->using_network_license = false;
			} else {
				$license = $this->get_network_license_key();
				if ( ! empty( $license ) ) {
					$this->licensed              = true;
	...
	public function using_network_license() {
		return $this->using_network_license;
	}

	public function get_license_key() {
		$license_key = UserFeedback()->license->get_site_license_key();
		$license_key = ! empty( $license_key ) ? $license_key : UserFeedback()->license->get_network_license_key();
		$license_key = ! empty( $license_key ) ? $license_key : UserFeedback()->license->get_default_license_key();
		return $license_key;
	}
	...
	public function license_can( $requires = 'lite' ) {
		if ( ! userfeedback_is_pro_version() || ! $this->has_license() ) {
	...
	public function get_default_license_key() {
		if ( defined( 'USERFEEDBACK_LICENSE_KEY' )
			&& is_string( USERFEEDBACK_LICENSE_KEY )
			&& strlen( USERFEEDBACK_LICENSE_KEY ) > 10 ) {
			return USERFEEDBACK_LICENSE_KEY;
		}
		return '';
	}
	...
	public function get_valid_license_key() {
		...
		} elseif ( $this->is_network_licensed() ) {
			return $this->get_network_license_key();
		} elseif ( $this->get_default_license_key() ) {
	...
	public function get_site_license_type() {
		return ( $this->get_site_license_key() && ! empty( $this->site['type'] )
				&& $this->is_valid_license_type( $this->site['type'] ) ) ? $this->site['type'] : '';
	}
	...
	public function site_license_can( $requires = 'lite' ) {
		...
		switch ( $requires ) {
			case 'elite':
				$can_access = $level === 'elite';
				break;

			case 'pro':
				$can_access = $level === 'master' || $level === 'pro';
				break;

			case 'plus':
				$can_access = $level === 'master' || $level === 'pro' || $level === 'plus';
				break;
	...
	public function site_license_has_error() {
		return $this->site_license_expired()  // is expired
	...
	public function network_license_has_error() {
		return $this->network_license_expired()  // is expired
	...
	public function get_site_license_updates() {
		return get_option( 'userfeedback_license_updates', '' );
	}
	public function get_network_license_updates() {
		return get_site_option( 'userfeedback_network_license_updates', '' );
	}

	public function set_site_license_updates() {
		update_option( 'userfeedback_license_updates', strtotime( '+8 hours' ) );
	}
	...
	public function time_to_check_site_license() {
		$timestamp = get_option( 'userfeedback_license_updates' );
		if ( ! $timestamp ) {
			return true;
		} else {
	...
	public function time_to_check_network_license() {
		$timestamp = get_site_option( 'userfeedback_network_license_updates' );
		if ( ! $timestamp ) {
			return true;
		} else {
			$current_timestamp = time();
	...
	public function set_site_license( $data = array() ) {
		update_option( 'userfeedback_license', $data );
		$this->set_site_license_updates();
		$this->site = $data;
	}
	public function set_network_license( $data = array() ) {
		update_site_option( 'userfeedback_network_license', $data );
