...
function userfeedback_is_plugin_our_addon( $plugin_file ) {
	...
	foreach ( $addons_data as $type => $addons ) {
		foreach ( $addons as $addon ) {
			...
			$basename = userfeedback_get_plugin_basename_from_slug( $slug );
 ...
 * @return string
 */
function userfeedback_modify_wordpress_autoupdater_setting( $html, $plugin_file, $plugin_data ) {
	$is_pro         = userfeedback_is_pro_version();
	$is_addon       = userfeedback_is_plugin_our_addon( $plugin_file );
	$is_main_free   = isset( $plugin_data['slug'] ) && 'userfeedback-pro' === $plugin_data['slug'];
	$is_main_pro    = $is_pro && plugin_basename( USERFEEDBACK_PLUGIN_FILE ) === $plugin_file;
	$has_permission = current_user_can( 'userfeedback_save_settings' );

	if ( $is_addon && ! $is_pro ) {
		$html = sprintf(
			'<a target="_blank" href="%s">%s</a>',
			userfeedback_get_url(
				'plugins-autoupdate',
...
add_filter( 'plugin_auto_update_setting_html', 'userfeedback_modify_wordpress_autoupdater_setting', 10, 3 );

/**
 * Filters the auto update plugin routine to allow UserFeedback to be
 ...
 */
function userfeedback_automatic_updates( $update, $item ) {
	$item = (array) $item;

	$is_free = isset( $item['slug'] ) && 'userfeedback-lite' === $item['slug'];
	$is_paid = isset( $item['userfeedback_plugin'] ); // see updater class

	$automatic_updates = userfeedback_get_option( 'automatic_updates', false );
	...
	if ( function_exists( 'get_current_screen' ) ) {
		...
		if ( ! empty( $screen ) &&
			...
			$is_pro      = userfeedback_is_pro_version();
			$is_addon    = userfeedback_is_plugin_our_addon( $item['plugin'] );
			$is_main_pro = $is_pro && plugin_basename( USERFEEDBACK_PLUGIN_FILE ) === $item['plugin'];
	...
	// variable will have `new_version`, `slug` and `userfeedback_plugin` filled out:
	...
	if ( ( ! $is_free && ! $is_paid ) || ( $is_free && ! defined( 'USERFEEDBACK_LITE_VERSION' ) ) ) {
		return $update;
	}

	$version       = $is_free ? USERFEEDBACK_LITE_VERSION : $item['old_version'];
	$current_major = userfeedback_get_major_version( $version );
	$new_major     = userfeedback_get_major_version( $item['new_version'] );
...
add_filter( 'auto_update_plugin', 'userfeedback_automatic_updates', 10, 2 );
/**
...
function userfeedback_get_major_version( $version ) {
