...
class UserFeedback_WP_Emails {
	...
	public function __construct( $template ) {
		...
		add_action( 'userfeedback_email_send_before', array( $this, 'send_before' ) );
		add_action( 'userfeedback_email_send_after', array( $this, 'send_after' ) );
	}
	...
	public function get_from_name() {
		...
		return apply_filters( 'userfeedback_email_from_name', userfeedback_decode_string( $this->from_name ), $this );
	}
	...
	public function get_from_address() {
		...
		return apply_filters( 'userfeedback_email_from_address', $this->from_address, $this );
	}
	...
	public function get_reply_to() {
		...
		return apply_filters( 'userfeedback_email_reply_to', $this->reply_to, $this );
	}
	...
	public function get_cc() {
		...
		return apply_filters( 'userfeedback_email_cc', $this->cc, $this );
	}
	...
	public function get_content_type() {
		...
		if ( ! $this->content_type && $this->html ) {
			$this->content_type = apply_filters( 'userfeedback_email_default_content_type', 'text/html', $this );
		} elseif ( ! $this->html ) {
			$this->content_type = 'text/plain';
		}

		return apply_filters( 'userfeedback_email_content_type', $this->content_type, $this );
	}
	...
	public function get_headers() {
		...
		return apply_filters( 'userfeedback_email_headers', $this->headers, $this );
	}
	...
	public function set_initial_args( $body = array() ) {
		...
		$args = apply_filters( 'userfeedback_emails_templates_set_initial_args', $args, $this );
	 ...
	 *
	 * @return UserFeedback_WP_Emails
	 */
	public function set_args( $args, $merge = true ) {

		$args = apply_filters( 'userfeedback_emails_templates_set_args', $args, $this );
	...
	public function get_args( $type ) {
		if ( ! empty( $type ) ) {
			return isset( $this->args[ $type ] ) ? apply_filters( 'userfeedback_emails_templates_get_args_' . $type, $this->args[ $type ], $this ) : array();
		}

		return apply_filters( 'userfeedback_emails_templates_get_args', $this->args, $this );
	}
	...
	public function build_email( $message = null ) {
		...
		if ( false === $this->html ) {
			...
			return apply_filters( 'userfeedback_email_message', $message, $this );
		}
	...
	public function send( $to, $subject, $message = null, $attachments = array() ) {
		...
		do_action( 'userfeedback_email_send_before', $this );
		 ...
		 */
		$data = apply_filters(
			'userfeedback_emails_send_email_data',
			array(
				'to'          => $to,
				'subject'     => $subject,
				'message'     => $message,
		...
		$sent = wp_mail(
			$data['to'],
			userfeedback_decode_string( $this->process_tag( $data['subject'] ) ),
		...
		do_action( 'userfeedback_email_send_after', $this );
	...
	public function process_tag( $string = '', $sanitize = true, $linebreaks = false ) {
		...
		$tag = apply_filters( 'userfeedback_process_smart_tags', $string );

		$tag = userfeedback_decode_string( $tag );

		if ( $sanitize ) {
			if ( $linebreaks ) {
				$tag = userfeedback_sanitize_textarea_field( $tag );
			} else {
	...
	public function is_email_disabled() {
		return (bool) apply_filters( 'userfeedback_disable_all_emails', false, $this );
	}
	...
	public function get_template() {
		...
		return apply_filters( 'userfeedback_email_template', $this->template );
	}
	...
	public function get_template_part( $slug, $name = null, $load = true ) {
		...
		if ( isset( $name ) ) {
			...
			return apply_filters( 'userfeedback_emails_templates_get_content_part', $html, $template, $this );
		}
	...
	public static function include_html( $template_name, $args = array(), $extract = false ) {
		...
		$located = apply_filters( 'userfeedback_helpers_templates_include_html_located', self::locate_template( $template_name ), $template_name, $args, $extract );
		$args    = apply_filters( 'userfeedback_helpers_templates_include_html_args', $args, $template_name, $extract );
		...
		$extract = apply_filters( 'userfeedback_helpers_templates_include_html_extract_args', $extract, $template_name, $args );
	...
	public static function locate_template( $template_name ) {
		...
		if ( empty( $template_name ) ) {
			return apply_filters( 'userfeedback_helpers_templates_locate', '', $template_name );
		}
		...
		return apply_filters( 'userfeedback_helpers_templates_locate', $located, $template_name );
	}
	...
	public static function get_theme_template_paths() {
		...
		$template_dir = 'userfeedback-email';

		$file_paths = array(
			1   => trailingslashit( get_stylesheet_directory() ) . $template_dir,
			10  => trailingslashit( get_template_directory() ) . $template_dir,
			100 => trailingslashit( USERFEEDBACK_PLUGIN_DIR ) . 'includes/emails/templates',
		);

		$file_paths = apply_filters( 'userfeedback_email_template_paths', $file_paths );
