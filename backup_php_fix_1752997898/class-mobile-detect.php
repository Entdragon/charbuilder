 ...
 * @method bool isWatch()
 */
class UserFeedback_Mobile_Detect {
...
class UserFeedback_Device_Detect {
	...
	public static function init() {
		 self::$detect = new UserFeedback_Mobile_Detect();
		self::$detect->setDetectionType( UserFeedback_Mobile_Detect::DETECTION_TYPE_EXTENDED );
		self::getIp();
	}
	...
	private static function getIp() {
		if ( isset( $_SERVER['HTTP_CLIENT_IP'] ) && ! empty( $_SERVER['HTTP_CLIENT_IP'] ) ) {
			self::$ipAddress = sanitize_text_field( wp_unslash( $_SERVER['HTTP_CLIENT_IP'] ) );
		} elseif ( isset( $_SERVER['HTTP_X_FORWARDED_FOR'] ) && ! empty( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ) {
			self::$ipAddress = sanitize_text_field( wp_unslash( $_SERVER['HTTP_X_FORWARDED_FOR'] ) );
		} else {
			self::$ipAddress = sanitize_text_field( wp_unslash( (!empty($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '' ) );
		}
	...
	public static function isEdge() {
		$agent = sanitize_text_field( wp_unslash( (!empty($_SERVER['HTTP_USER_AGENT'])) ? $_SERVER['HTTP_USER_AGENT'] : '' ) );
	...
	public static function os() {
		...
		if ( self::$detect->isAndroidOS() ) {
			if ( self::$detect->version( 'Android' ) !== false ) {
				...
				switch ( true ) {
					case self::$detect->version( 'Android' ) >= 9.0:
						$codeName = ' (Pie)';
						break;
					case self::$detect->version( 'Android' ) >= 8.0:
						$codeName = ' (Oreo)';
						break;
					case self::$detect->version( 'Android' ) >= 7.0:
						$codeName = ' (Nougat)';
						break;
					case self::$detect->version( 'Android' ) >= 6.0:
						$codeName = ' (Marshmallow)';
						break;
					case self::$detect->version( 'Android' ) >= 5.0:
						$codeName = ' (Lollipop)';
						break;
					case self::$detect->version( 'Android' ) >= 4.4:
						$codeName = ' (KitKat)';
						break;
					case self::$detect->version( 'Android' ) >= 4.1:
						$codeName = ' (Jelly Bean)';
						break;
					case self::$detect->version( 'Android' ) >= 4.0:
						$codeName = ' (Ice Cream Sandwich)';
						break;
					case self::$detect->version( 'Android' ) >= 3.0:
						$codeName = ' (Honeycomb)';
						break;
					case self::$detect->version( 'Android' ) >= 2.3:
						$codeName = ' (Gingerbread)';
						break;
					case self::$detect->version( 'Android' ) >= 2.2:
						$codeName = ' (Froyo)';
						break;
					case self::$detect->version( 'Android' ) >= 2.0:
						$codeName = ' (Eclair)';
						break;
					case self::$detect->version( 'Android' ) >= 1.6:
						$codeName = ' (Donut)';
						break;
					case self::$detect->version( 'Android' ) >= 1.5:
						$codeName = ' (Cupcake)';
						break;
					default:
						$codeName = '';
			...
			switch ( self::$detect->version( 'Windows NT' ) ) {
				...
				case 4.0:
					...
					break;
				default:
					$codeName = ' NT v' . self::$detect->version( 'Windows NT' );
					break;
			}
			$os = 'Windows' . $codeName;
	...
	private static $phoneModels = array(
		'Samsung' => array(
...
UserFeedback_Device_Detect::init();
