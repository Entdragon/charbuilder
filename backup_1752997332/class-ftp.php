...
class ftp_base {
	...
	/* Constructor */
	function CG_Dev\\__construct($port_mode=FALSE, $verb=FALSE, $le=FALSE) {
		$this->LocalEcho=$le;
		$this->Verbose=$verb;
	...
	function CG_Dev\\ftp_base($port_mode=FALSE) {
...
// <!-- --------------------------------------------------------------------------------------- -->
	...
	function CG_Dev\\parselisting($line) {
	...
	function CG_Dev\\SendMSG($message = "", $crlf=true) {
		if ($this->Verbose) {
	...
	function CG_Dev\\SetType($mode=FTP_AUTOASCII) {
	...
	function CG_Dev\\_settype($mode=FTP_ASCII) {
		if($this->_ready) {
			if($mode==FTP_BINARY) {
	...
	function CG_Dev\\Passive($pasv=NULL) {
	...
	function CG_Dev\\SetServer($host, $port=21, $reconnect=true) {
		if(!is_long($port)) {
	        $this->verbose=true;
	...
	function CG_Dev\\SetUmask($umask=0022) {
		$this->_umask=$umask;
		umask($this->_umask);
	...
	function CG_Dev\\SetTimeout($timeout=30) {
	...
	function CG_Dev\\connect($server=NULL) {
		if(!empty($server)) {
	...
	function CG_Dev\\quit($force=false) {
		if($this->_ready) {
	...
	function CG_Dev\\login($user=NULL, $pass=NULL) {
	...
	function CG_Dev\\filesize($pathname) {
	...
	function CG_Dev\\delete($pathname) {
		if(!$this->_exec("DELE ".$pathname, "delete")) return FALSE;
		if(!$this->_checkCode()) return FALSE;
		return TRUE;
	}

	function CG_Dev\\site($command, $fnction="site") {
		if(!$this->_exec("SITE ".$command, $fnction)) return FALSE;
		if(!$this->_checkCode()) return FALSE;
		return TRUE;
	}

	function CG_Dev\\chmod($pathname, $mode) {
		if(!$this->site( sprintf('CHMOD %o %s', $mode, $pathname), "chmod")) return FALSE;
		return TRUE;
	}

	function CG_Dev\\restore($from) {
	...
	function CG_Dev\\rawlist($pathname="", $arg="") {
	...
	function CG_Dev\\is_exists($pathname) {
		return $this->file_exists($pathname);
	}

	function CG_Dev\\file_exists($pathname) {
		$exists=true;
	...
	function CG_Dev\\fget($fp, $remotefile, $rest=0) {
	...
	function CG_Dev\\get($remotefile, $localfile=NULL, $rest=0) {
	...
	function CG_Dev\\fput($remotefile, $fp, $rest=0) {
		if($this->_can_restore and $rest!=0) fseek($fp, $rest);
		$pi=pathinfo($remotefile);
		if($this->_type==FTP_ASCII or ($this->_type==FTP_AUTOASCII and in_array(strtoupper($pi["extension"]), $this->AutoAsciiExt))) $mode=FTP_ASCII;
		else $mode=FTP_BINARY;
	...
	function CG_Dev\\put($localfile, $remotefile=NULL, $rest=0) {
	...
	function CG_Dev\\mput($local=".", $remote=NULL, $continious=false) {
	...
	function CG_Dev\\mget($remote, $local=".", $continious=false) {
	...
	function CG_Dev\\mdel($remote, $continious=false) {
		$list=$this->rawlist($remote, "-la");
		if($list===false) {
	...
	function CG_Dev\\mmkdir($dir, $mode = 0777) {
	...
	function CG_Dev\\glob($pattern, $handle=NULL) {
		$path=$output=null;
	...
	function CG_Dev\\glob_pattern_match($pattern,$subject) {
		$out=null;
	...
	function CG_Dev\\glob_regexp($pattern,$subject) {
	...
	function CG_Dev\\dirlist($remote) {
		$list=$this->rawlist($remote, "-la");
		if($list===false) {
...
// <!--       Private functions                                                                 -->
// <!-- --------------------------------------------------------------------------------------- -->
	function CG_Dev\\_checkCode() {
		return ($this->_code<400 and $this->_code>0);
	}

	function CG_Dev\\_list($arg="", $cmd="LIST", $fnction="_list") {
...
// Gnre une erreur pour traitement externe  la classe
	function CG_Dev\\PushError($fctname,$msg,$desc=false){
		$error=array();
		$error['time']=time();
