...
(function() {
  ...
  function loadChar(id) {
    ajaxPost('uj_get_character', { id: id }).then(function(res) {
      ...
      state.powerChoices   = (function() {
        try { return JSON.parse(c.power_choices || '{}') || {}; } catch(e) { return {}; }
      })();
  ...
  function buildGiftsStep() {
    ...
    gifts.forEach(function(g) {
      ...
      if (isPowerGate) {
        ...
        var pickHint = (g.slug === 'personal-power')
          ? 'Personal Power supplies your power die. Pick which Power(s) you can call upon.'
          : 'Petitioned Power lets you petition a spirit. Pick which Power(s) you can access through it.';
        ...
        pmeta.forEach(function(pm) {
          ...
          html += '<label class="uj-power-pill" data-power-key="' + esc(pm.key) + '" style="cursor:pointer;padding:0.35rem 0.9rem;border-radius:999px;font-size:0.82rem;user-select:none;' +
            'border:1px solid ' + (active ? 'var(--uj-amber)' : 'var(--uj-border-cool)') + ';' +
            'color:' + (active ? 'var(--uj-amber-light)' : 'var(--uj-text-muted)') + ';' +
            'background:' + (active ? 'rgba(180,120,30,0.18)' : 'transparent') + ';">' +
            '<input type="checkbox" value="' + esc(pm.key) + '"' + (active ? ' checked' : '') + ' style="display:none;">' +
            esc(pm.label) + '</label>';
        });
        html += '</div></div>';
      }
  ...
  function buildSummaryStep() {
    ...
    // ── Powers section ─────────────────────────────────────────────
    // Combine: (a) Powers auto-granted by Species/Type/Career (from server
    // resolver) and (b) Powers picked via Personal Power / Petitioned Power.
    ...
    [sp, ty, ca, extraTy, extraCa].forEach(function(src) {
      if (!src || !src.powers) return;
      src.powers.forEach(function(pm) { addPower(pm, src.name); });
    });
    // Picks via Personal/Petitioned Power gifts — only honor picks whose
    // gate gift is still present in the current build (avoids stale picks
    // surviving a career/type change).
    var giftMetaByKey = {};
    powersMeta.forEach(function(pm) { giftMetaByKey[pm.key] = pm; });
    var activeGateKeys = {};
    collectAllGifts().forEach(function(gg) {
      if (gg.slug === 'personal-power' || gg.slug === 'petitioned-power') {
        activeGateKeys[gg.slug + '_' + gg.occurrence] =
          (gg.slug === 'personal-power') ? 'Personal Power' : 'Petitioned Power';
      }
    });
    Object.keys(activeGateKeys).forEach(function(choiceKey) {
      ...
      picks.forEach(function(k) {
        if (giftMetaByKey[k]) addPower(giftMetaByKey[k], label);
      });
    });
    ...
    if (powerList.length) {
      ...
      powerList.forEach(function(p) {
        var pm = p.meta;
        var effects = powerEffects.filter(function(e) { return e.power === pm.key; });
        ...
        if (effects.length) {
          ...
          html += '<thead><tr>' +
            '<th style="width:2.4rem;text-align:left;">Ord</th>' +
            '<th style="text-align:left;">Effect</th>' +
            '<th style="text-align:left;width:30%;">Descriptors</th>' +
            '<th style="text-align:left;">Description</th>' +
          '</tr></thead><tbody>';
          effects.forEach(function(e) {
            var ord = ['I','II','III','IV','V','VI','VII','VIII','IX','X'][(e.order_level|0) - 1] || String(e.order_level || '');
            html += '<tr>' +
              '<td style="color:var(--uj-amber-light);font-weight:600;">' + esc(ord) + '</td>' +
              '<td style="font-weight:500;">' + esc(e.name) + '</td>' +
              '<td style="font-size:0.74rem;color:var(--uj-text-dim);">' + esc(e.descriptors || '') + '</td>' +
              '<td style="font-size:0.78rem;color:var(--uj-text-muted);line-height:1.45;">' + esc(e.description || '') + '</td>' +
            '</tr>';
          });
