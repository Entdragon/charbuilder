  ...
  var TraitsService = {
    ...
    updateAdjustedDisplays() {
      ...
      TRAITS.forEach((traitKey) => {
        ...
        $24(`#cg-${traitKey}-adjusted`).text(label + suffix);
      });
      console.groupEnd();
    },
    refreshAll() {
...
<form id="cg-form">
  ...
  var render_default = {
    render() {
      console.group("[SkillsRender] \u{1F9E9} render() called");
      const skills = window.CG_SKILLS_LIST || [];
      const species = api_default.currentProfile || {};
      const career = api_default2.currentProfile || {};
      const data = formBuilder_default.getData();
      console.log("[SkillsRender] \u{1F9EC} Loaded species profile:", species);
      console.log("[SkillsRender] \u{1F9EC} Loaded career profile:", career);
      console.log("[SkillsRender] \u{1F4E6} Current builder data:", data);
      ...
      $5("#marks-remaining").remove();
      $5("#skills-table").before(`
      <div id="marks-remaining" class="marks-remaining">
        Marks Remaining: <strong>${marksRemain}</strong>
      </div>
    `);
      const $thead = $5("<thead>");
      $5("<tr>").append("<th>Skill</th>").append(`<th>${species.speciesName || ""}</th>`).append(`<th>${career.careerName || ""}</th>`).append("<th>Marks</th>").append("<th>Dice Pool</th>").appendTo($thead);
      ...
      skills.forEach((skill) => {
        ...
        console.log(`[SkillsRender] \u{1F3AF} Skill "${name}" (ID: ${id})`);
        ...
        const $row = $5("<tr>").append(`<td>${name}</td>`).append(`<td>${spDie || "\u2013"}</td>`).append(`<td>${cpDie || "\u2013"}</td>`).append(`<td>
                   <div class="marks-buttons">${buttonsHtml}</div>
                   <div class="marks-display">${markDisplay}</div>
                 </td>`).append(`<td>${poolStr}</td>`);
        $tbody.append($row);
      });
      console.log("[SkillsRender] \u{1F9F1} Injecting built table into #skills-table");
      $5("#skills-table").empty().append($thead).append($tbody);
      console.groupEnd();
    }
  };
  ...
  var $6 = window.jQuery;
  console.log("\u{1F525} [SkillsEvents] Module loaded");
  var events_default = {
    bind() {
      console.group("[SkillsEvents] \u{1F9F7} bind() called");
      console.log("[SkillsEvents] \u{1F4CE} Binding click/change on #tab-skills, #cg-species, #cg-career");
      $6(document).off("click", "#tab-skills, #cg-species, #cg-career").on("click change", "#tab-skills, #cg-species, #cg-career", () => {
        console.log("[SkillsEvents] \u{1F504} Tab or selector change detected \u2014 calling SkillsRender.render()");
        render_default.render();
      });
      console.log("[SkillsEvents] \u{1F4CE} Binding click on .skill-mark-btn");
      $6(document).off("click", ".skill-mark-btn").on("click", ".skill-mark-btn", function() {
        const skillId = String($6(this).data("skill-id"));
        const mark = parseInt($6(this).data("mark"), 10);
        const data = formBuilder_default.getData();
        console.log(`[SkillsEvents] \u{1F3AF} Clicked skill mark button \u2192 Skill ID: ${skillId}, Mark: ${mark}`);
        console.log("[SkillsEvents] \u{1F4E6} Current skillMarks before update:", data.skillMarks);
        data.skillMarks = data.skillMarks || {};
        data.skillMarks[skillId] = mark;
        console.log("[SkillsEvents] \u2705 Updated skillMarks:", data.skillMarks);
        console.log("[SkillsEvents] \u{1F501} Calling SkillsRender.render() after mark update");
        render_default.render();
      });
      console.groupEnd();
    }
  ...
  console.log("\u{1F525} [SkillsIndex] Module loaded");
  var skills_default = {
    init() {
      console.group("[SkillsIndex] \u{1F680} init() called");
      console.log("[SkillsIndex] \u{1F9FE} Initial FormBuilderAPI._data:", formBuilder_default._data);
      if (!Array.isArray(formBuilder_default._data.skillsList)) {
        console.log("[SkillsIndex] \u{1F4E5} No skillsList found, pulling from CG_SKILLS_LIST...");
        formBuilder_default._data.skillsList = window.CG_SKILLS_LIST || [];
      } else {
        console.log("[SkillsIndex] \u2705 Existing skillsList found");
      }
      console.log("[SkillsIndex] \u{1F4E6} skillsList:", formBuilder_default._data.skillsList);
      ...
      console.log("[SkillsIndex] \u{1F9EE} skillMarks:", formBuilder_default._data.skillMarks);
      console.log("[SkillsIndex] \u{1F9F7} Binding event handlers via SkillsEvents.bind()");
      events_default.bind();
      console.log("[SkillsIndex] \u{1F5BC}\uFE0F Rendering UI via SkillsRender.render()");
      render_default.render();
      console.groupEnd();
    }
  };
  ...
  function bind() {
    ...
    bound = true;
    console.group("[TraitsEvents] \u{1F517} Binding Trait Events");
    $7(function() {
      console.log("[TraitsEvents] \u2705 Page ready \u2192 Initial TraitsService.refreshAll()");
      service_default.refreshAll();
    });
    $7(document).off("change", ".cg-trait-select").on("change", ".cg-trait-select", function() {
      const traitId = $7(this).attr("id");
      const value = $7(this).val();
      console.log(`[TraitsEvents] \u{1F504} Trait changed \u2192 ID: ${traitId}, New Value: ${value}`);
      service_default.refreshAll();
    });
    $7(document).off("cg-profile-updated.traits").on("cg-profile-updated.traits", () => {
      console.log("[TraitsEvents] \u{1F514} Event: cg-profile-updated.traits \u2192 TraitsService.refreshAll()");
      service_default.refreshAll();
    });
    console.groupEnd();
  }
  var TraitsEvents = { bind };
  var events_default2 = TraitsEvents;

  // assets/js/src/core/traits/index.js
  console.log("\u{1F525} [Traits] Module loaded");
  var traits_default = {
    init() {
      console.group("[Traits] \u{1F501} init()");
    ...
    bind() {
      ...
      $9(document).off("change", "#cg-species").on("change", "#cg-species", () => {
        ...
        if (!speciesId) {
          ...
          render_default2.clearUI();
          state_default.setList([]);
          service_default.refreshAll();
          return;
        }
        api_default.loadSpeciesProfile(speciesId, (profileData) => {
          console.log("[SpeciesEvents] \u{1F4E6} Loaded species profile \u2192", profileData);
          ...
          console.log("[SpeciesEvents] \u{1F9EC} Species gifts parsed \u2192", spGifts);
          state_default.setList(spGifts);
          console.log("[SpeciesEvents] \u2705 GiftState set. Now rendering and refreshing...");
          render_default2.renderGifts(profileData);
          service_default.refreshAll();
        });
      });
    }
  ...
  var events_default4 = {
    ...
    handleCareerChange() {
      ...
      api_default2.loadGifts(careerId, (profile) => {
        console.log("[CareerEvents] \u{1F4E6} Loaded career profile \u2192", profile);
        ...
        console.log("[CareerEvents] \u{1F9EC} Parsed career gifts \u2192", crGifts);
        ...
        if (sp.gift_id_1 || sp.gift_id_2 || sp.gift_id_3) {
          console.log("[CareerEvents] \u{1F501} Re-rendering species gifts to preserve UI");
          render_default2.renderGifts(sp);
        }
        service_default.refreshAll();
        render_default.render();
      });
  ...
  var SummaryAPI = {
     ...
     */
    renderSummary(data = {}) {
      const $sheet = $18("#cg-summary-sheet").empty();
     ...
     */
    bindExportButton() {
      $18(document).off("click", "#cg-export-pdf").on("click", "#cg-export-pdf", (e) => {
        e.preventDefault();
  ...
  function refreshTab() {
    console.group("[BuilderRefresh] \u{1F501} refreshTab()");
    const tab = $19("#cg-modal .cg-tabs li.active").data("tab");
    console.log(`[BuilderRefresh] \u{1F4CC} Active tab detected: ${tab}`);
    switch (tab) {
      case "tab-traits":
        console.log("[BuilderRefresh] \u{1F527} Initializing Traits tab...");
        traits_default.init();
        console.log("[BuilderRefresh] \u2705 TraitsAPI.init() complete");
        break;
      case "tab-profile":
        console.log("[BuilderRefresh] \u{1F527} Initializing Profile tab...");
        ...
        break;
      case "tab-skills":
        console.log("[BuilderRefresh] \u{1F527} Initializing Skills tab...");
        skills_default.init();
        console.log("[BuilderRefresh] \u2705 SkillsAPI.init() complete");
        break;
      case "tab-summary":
        console.log("[BuilderRefresh] \u{1F527} Initializing Summary tab...");
  ...
  function bindLoadEvents() {
    $20(document).off("click", "#cg-load-confirm").on("click", "#cg-load-confirm", (e) => {
      e.preventDefault();
    ...
    $20(document).off("click", "#cg-splash-back").on("click", "#cg-splash-back", (e) => {
      e.preventDefault();
      console.log("[BuilderLoad] #cg-splash-back clicked \u2192 show NEW form");
      bindUIEvents.openBuilder({ isNew: true });
    });
  }
  ...
  function bindSaveEvents() {
    $21(document).off("click", ".cg-save-button").on("click", ".cg-save-button", function(e) {
      e.preventDefault();
      const closeAfter = $21(this).hasClass("cg-close-after-save");
      const payload = formBuilder_default.collectFormData();
      console.log("[BuilderSave] \u25B6 saving payload:", payload);
      $21.post(CG_Ajax.ajax_url, {
        action: "cg_save_character",
        character: payload,
        security: CG_Ajax.nonce
      }).done((res) => {
        ...
        bindUIEvents.markClean();
        if (closeAfter) {
          bindUIEvents.closeBuilder();
        } else {
  ...
  window.TraitsService = service_default;
  var $22 = window.jQuery;
  function bindUIEvents() {
    console.log("[BuilderEvents] \u{1F527} bindUIEvents() called");
    $22(document).off("input change", "#cg-modal input, #cg-modal select, #cg-modal textarea").on("input change", "#cg-modal input, #cg-modal select, #cg-modal textarea", function() {
      bindUIEvents.markDirty();
      const $el = $22(this);
      ...
      if ($el.hasClass("skill-marks")) {
        ...
        console.log(`[BuilderEvents] \u{1F522} Updated skillMarks \u2192`, formBuilder_default._data.skillMarks);
        return;
      }
      if (!this.id) {
      ...
      console.log(`[BuilderEvents] \u{1F4BE} Updated FormBuilderAPI._data[${key}] \u2192`, $el.val());
    });
    $22(document).off("click", "#cg-open-builder").on("click", "#cg-open-builder", (e) => {
      e.preventDefault();
      console.log("[BuilderEvents] \u{1F4C2} Open builder splash");
      $22("#cg-modal-splash").removeClass("cg-hidden").addClass("visible");
      formBuilder_default.listCharacters().done((resp) => {
        console.log("[BuilderEvents] \u{1F9FE} Character list fetched:", resp.data);
        const $sel = $22("#cg-splash-load-select").empty();
        resp.data.forEach((c) => {
    ...
    $22(document).off("click", "#cg-new-splash").on("click", "#cg-new-splash", (e) => {
      e.preventDefault();
      console.log("[BuilderEvents] \u{1F195} Starting new character");
      ...
      console.log("[BuilderEvents] \u{1F504} Initializing blank payload:", payload);
      formBuilder_default._data = __spreadValues({}, payload);
      formBuilder_default._data.skillMarks = {};
      formBuilder_default._data.species = "";
      formBuilder_default._data.career = "";
      if (window.CG_FreeChoicesState) {
        console.log("[BuilderEvents] \u{1F9F9} Resetting FreeChoicesState");
      ...
      bindUIEvents.openBuilder({ isNew: true, payload });
      requestAnimationFrame(() => {
        console.log("[BuilderEvents] \u{1F501} TraitsService.refreshAll() (new character)");
        service_default.refreshAll();
      });
    });
    $22(document).off("click", "#cg-load-splash").on("click", "#cg-load-splash", (e) => {
      e.preventDefault();
      ...
      console.log(`[BuilderEvents] \u{1F4E5} Loading character ID: ${charId}`);
      formBuilder_default.fetchCharacter(charId).done((resp) => {
        ...
        $22("#cg-modal-splash").removeClass("visible").addClass("cg-hidden");
        bindUIEvents.openBuilder({ isNew: false, payload: record });
        requestAnimationFrame(() => {
          console.log("[BuilderEvents] \u{1F501} TraitsService.refreshAll() (loaded character)");
          service_default.refreshAll();
        });
      }).fail((xhr, status, err) => {
    ...
    console.log("[BuilderEvents] \u{1F517} Binding load/save events");
    bindLoadEvents();
    bindSaveEvents();
    $22(document).off("click", "#cg-modal .cg-tabs li").on("click", "#cg-modal .cg-tabs li", function(e) {
      e.preventDefault();
      const tabName = $22(this).data("tab");
      console.log(`[BuilderEvents] \u{1F9ED} Switching to tab: ${tabName}`);
    ...
    $22(document).off("click", "#cg-modal-close").on("click", "#cg-modal-close", (e) => {
      e.preventDefault();
      console.log("[BuilderEvents] \u{1F9E8} Modal close clicked \u2014 checking unsaved changes");
      bindUIEvents.showUnsaved();
    });
    $22(document).off("click", "#cg-modal-overlay").on("click", "#cg-modal-overlay", function(e) {
      if (e.target !== this)
        return;
      console.log("[BuilderEvents] \u{1F9E8} Modal overlay clicked \u2014 checking unsaved changes");
      bindUIEvents.showUnsaved();
    });
    $22(document).off("click", "#unsaved-save").on("click", "#unsaved-save", (e) => {
      e.preventDefault();
      console.log("[BuilderEvents] \u{1F4BE} Save & Close clicked");
      formBuilder_default.save(true);
    });
    $22(document).off("click", "#unsaved-exit").on("click", "#unsaved-exit", (e) => {
      e.preventDefault();
      console.log("[BuilderEvents] \u274C Exit without saving clicked");
      bindUIEvents.closeBuilder();
    });
    $22(document).off("click", "#unsaved-cancel").on("click", "#unsaved-cancel", (e) => {
      e.preventDefault();
      console.log("[BuilderEvents] \u{1F519} Cancel exit clicked");
      bindUIEvents.hideUnsaved();
    });
  }

  // assets/js/src/core/main/index.js
  function bindUIEvents2() {
    skills_default.init();
    (0, import_jquery.default)(document).on("click", ".cg-open-builder", (e) => {
      e.preventDefault();
      bindUIEvents.openBuilder({ isNew: true, payload: {} });
    });
    (0, import_jquery.default)(document).on("click", ".cg-edit-character", (e) => {
      e.preventDefault();
      const id = (0, import_jquery.default)(e.currentTarget).data("character-id");
      formBuilder_default.fetchCharacter(id).done((character) => {
        bindUIEvents.openBuilder({ isNew: false, payload: character });
      });
    });
  }
  var main_default = {
