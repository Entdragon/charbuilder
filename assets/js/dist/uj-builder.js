(() => {
  ...
  (function() {
    ...
    if (cpwCancel)
      cpwCancel.addEventListener("click", closeChangePw);
    if (changePwOverlay)
    ...
    if (cpwSubmit)
      cpwSubmit.addEventListener("click", function() {
        var cur = document.getElementById("cpw-current").value;
        var nw = document.getElementById("cpw-new").value;
        var confirm2 = document.getElementById("cpw-confirm").value;
        if (cpwError)
          cpwError.textContent = "";
        ...
        ajaxPost("cg_change_password", {
          ...
          if (res.success) {
            ...
            if (cpwError)
              cpwError.textContent = res.data || "Something went wrong.";
          }
        }).catch(function() {
          cpwSubmit.disabled = false;
          cpwSubmit.textContent = "Update Password";
          if (cpwError)
            cpwError.textContent = "Request failed \u2014 please try again.";
        });
      });
    var state = {
      allData: null,
    ...
    function renderDevelop() {
      ...
      function getDieForTrait(traitLabel) {
        var map = {
          "Body": state.bodyDie,
          "Speed": state.speedDie,
          "Mind": state.mindDie,
          "Will": state.willDie,
          "Species": state.speciesDie,
          "Type": state.typeDie,
          "Career": state.careerDie,
      ...
      function buildShopCard(item, kind) {
        ...
        var btnLabel = fromCreation ? "Granted at creation" : atMaxDie ? "At maximum (d12)" : alreadyOwned ? "Already owned" : !canAfford ? "Need 10 XP" : "Buy \u2014 10 XP";
      ...
      var ownedSlugSet = {};
      Object.keys(creationGrantedSlugs).forEach(function(s) {
        ownedSlugSet[s] = true;
      });
      state.purchasedGifts.forEach(function(p) {
        ownedSlugSet[p.slug] = true;
      });
      function buildExtraTraitCard(g) {
        ...
        if (selectedId) {
          var found = candidateList.find(function(x) {
        ...
        if (alreadyPurchased)
          html += ' <span class="dev-badge dev-badge-owned">Purchased</span>';
        html += "</div></div>";
        if (g.subtitle)
          html += '<div style="font-size:0.78rem;color:var(--uj-amber-light);font-style:italic;margin-bottom:0.15rem;">' + esc(g.subtitle) + "</div>";
        if (g.description)
          html += '<p class="dev-shop-card-desc">' + esc(g.description) + "</p>";
        if (alreadyPurchased) {
          var effDie = effectiveDie(selectedDie, isCareer ? "Extra Career" : "Extra Type");
          html += '<div style="padding:0.4rem 0;font-size:0.85rem;color:var(--uj-teal);">Chosen: <strong>' + esc(selectedName) + "</strong> \u2014 die: <strong>" + esc(effDie) + "</strong>" + (effDie !== selectedDie ? ' <span style="font-size:0.75rem;color:var(--uj-text-dim);">(base ' + esc(selectedDie) + ")</span>" : "") + "</div>";
        } else {
          html += '<div style="margin:0.6rem 0;">';
          if (eligibleItems.length === 0) {
            html += '<div style="font-size:0.82rem;color:var(--uj-text-dim);font-style:italic;">No eligible ' + kindLabel + "s yet \u2014 you must first purchase all creation gifts" + (isCareer ? "" : " and soaks") + " from a " + kindLabel + " through development.</div>";
          } else {
          ...
          html += '<div class="dev-shop-card-footer"><button class="uj-btn uj-btn-teal" style="font-size:0.75rem;padding:0.3rem 0.85rem;" data-buy-slug="' + esc(g.slug) + '" data-buy-name="' + esc(g.name) + '" data-buy-kind="Gift"' + btnDisabled2 + ">" + btnLabel2 + "</button></div>";
        }
        html += "</div>";
        return html;
      }
      var IMPROVED_TRAITS = ["Body", "Speed", "Mind", "Will", "Career", "Species", "Type"];
      if (state.extraCareerId !== null)
      ...
      developScreen.querySelectorAll("[data-buy-slug]").forEach(function(btn) {
        btn.addEventListener("click", function() {
          ...
          if (slug === "extra-career" || slug === "extra-type") {
            ...
            if (!confirm('Spend 10 XP to purchase "' + name + ": " + selectedName + '"?'))
              return;
            ...
            state.purchasedGifts.push({ slug, name: name + ": " + selectedName, kind, xp_cost: 10, purchased_on: today });
            saveDevelop(renderDevelop);
            return;
          }
    ...
    function buildSummaryStep() {
      ...
      function skillDiceFor(skillName) {
        ...
        if (itemGrantsSkill(extraCa, skillName) && effExtraCaDie)
          dice.push({ die: effExtraCaDie, src: "Extra Career" });
        if (itemGrantsSkill(extraTy, skillName) && effExtraTyDie)
          dice.push({ die: effExtraTyDie, src: "Extra Type" });
        return dice;
      }
      var giftMap = {};
      ...
      CORE_SKILLS.forEach(function(skillName) {
        ...
        var dieBadge = function(d2) {
          return d2 ? '<span class="skill-die-badge">' + d2 + "</span>" : '<span class="skill-die-empty">\u2014</span>';
        };
        html += '<tr class="' + (hasAny ? "skill-row-active" : "skill-row-empty") + '"><td class="skill-name-col">' + esc(skillName) + '</td><td class="skill-die-col">' + dieBadge(spDie) + '</td><td class="skill-die-col">' + dieBadge(tyDie) + '</td><td class="skill-die-col">' + dieBadge(caDie) + "</td>" + (extraCa ? '<td class="skill-die-col">' + (exCaDie ? '<span class="skill-die-badge" style="color:var(--uj-teal);border-color:var(--uj-teal);">' + exCaDie + "</span>" : '<span class="skill-die-empty">\u2014</span>') + "</td>" : "") + (extraTy ? '<td class="skill-die-col">' + (exTyDie ? '<span class="skill-die-badge" style="color:var(--uj-teal);border-color:var(--uj-teal);">' + exTyDie + "</span>" : '<span class="skill-die-empty">\u2014</span>') + "</td>" : "") + '<td class="skill-total-col">' + (pool.length ? '<span style="color:var(--uj-teal);font-weight:600;">' + pool.join(" + ") + "</span>" : '<span class="skill-die-empty">\u2014</span>') + "</td></tr>";
      });
      ...
      [sp, ty, ca, extraCa, extraTy].forEach(function(entity) {
        ...
        entity.gifts.forEach(function(g) {
          charGiftSlugs[g.slug || String(g.id)] = true;
        });
      });
      state.purchasedGifts.forEach(function(p) {
        charGiftSlugs[p.slug] = true;
      });
      var charAccessible = { "unarmed": true };
      GIFT_ATTACK_CATS.forEach(function(cat) {
        if (charGiftSlugs[cat])
          charAccessible[cat] = true;
      });
      var charTraitMap = {
      ...
      function parseAttackPool(diceStr, traitMap, skillFn) {
        ...
        str.split(",").forEach(function(part) {
          ...
          if (!part)
            return;
          if (/^d\d+$/i.test(part)) {
            pool.push(part.toLowerCase());
            return;
          }
          if (/^ammo\s+d\d+$/i.test(part)) {
            pool.push(part.replace(/^ammo\s+/i, "").toLowerCase());
            return;
          }
          if (traitMap[part]) {
            pool.push(traitMap[part]);
            return;
          }
          var sd = skillFn(part);
      ...
      var charAttacks = allAttacks.filter(function(a) {
        return charAccessible[a.category];
      });
      if (charAttacks.length) {
        ...
        html += '<thead><tr><th class="skill-name-col">Attack</th><th style="width:5rem;text-align:left;padding-left:0.4rem;">Range</th><th style="text-align:left;padding-left:0.4rem;">Dice Pool</th><th style="text-align:left;padding-left:0.4rem;">Effect</th></tr></thead><tbody>';
        var lastCat = "";
        charAttacks.forEach(function(a) {
          if (a.category !== lastCat) {
            lastCat = a.category;
            html += '<tr><td colspan="4" style="font-size:0.68rem;text-transform:uppercase;letter-spacing:0.1em;color:var(--uj-amber-light);padding:0.55rem 0.4rem 0.2rem;border-top:1px solid var(--uj-border-cool);">' + esc(lastCat.charAt(0).toUpperCase() + lastCat.slice(1).replace(/-/g, " ")) + "</td></tr>";
          }
          var pool = parseAttackPool(a.attack_dice, charTraitMap, skillDiceFor);
          html += '<tr class="skill-row-active"><td class="skill-name-col" style="font-weight:500;">' + esc(a.name) + '</td><td style="font-size:0.78rem;color:var(--uj-text-dim);padding-left:0.4rem;">' + esc(a.attack_range) + '</td><td style="padding-left:0.4rem;">' + (pool.length ? '<span style="color:var(--uj-teal);font-weight:600;">' + pool.join(" + ") + "</span>" : '<span class="skill-die-empty">\u2014</span>') + '</td><td style="font-size:0.78rem;color:var(--uj-text-dim);padding-left:0.4rem;">' + esc(a.effect) + "</td></tr>";
        });
        html += "</tbody></table></div>";
      }
      ...
      var hasAllyFromCreation = !!(giftMap["_ally"] || Object.values(giftMap).some(function(g) {
        return g.slug === "ally";
      }));
      var hasDevAlly = state.purchasedGifts && state.purchasedGifts.some(function(p) {
        return p.slug === "ally";
      });
      if ((hasAllyFromCreation || hasDevAlly) && (state.allySpeciesId || state.allyCareerId || state.allyName)) {
        let allyDiceBlock = function(label, die, improved) {
        ...
        }, addAllyGiftSlugs = function(entity) {
          if (!entity || !entity.gifts)
            return;
          entity.gifts.forEach(function(g) {
            allyGiftSlugsForAtk[g.slug || String(g.id)] = true;
          });
        }, allySkillFn = function(skillName) {
          var dice = [];
          if (allyGrantsSkill(allySp, skillName) && aSpDie)
            dice.push({ die: aSpDie });
          if (allyGrantsSkill(allyCa, skillName) && aCaDie)
            dice.push({ die: aCaDie });
          return dice;
          ...
          entity.gifts.forEach(function(g) {
            ...
            if (!allyGiftSeen[key]) {
              allyGiftSeen[key] = true;
              allyGifts.push(g);
            }
          });
        }, addAllySoaks = function(entity) {
          if (!entity || !entity.soaks)
            return;
          entity.soaks.forEach(function(s) {
            var key = s.slug || String(s.id);
            if (!allySoakSeen[key]) {
              allySoakSeen[key] = true;
              allySoaks.push(s);
            }
          });
        };
        var allySp = state.allySpeciesId ? (d.species || []).find(function(x) {
          return x.id == state.allySpeciesId;
        }) : null;
        var allyCa = state.allyCareerId ? (d.careers || []).find(function(x) {
          return x.id == state.allyCareerId;
        }) : null;
        ...
        GIFT_ATTACK_CATS.forEach(function(cat) {
          if (allyGiftSlugsForAtk[cat])
            allyAccessible[cat] = true;
        });
        var allyTraitMap = { "Body": aBodyDie, "Speed": aSpeedDie, "Mind": aMindDie, "Will": aWillDie };
        var allyAttackList = allAttacks.filter(function(a) {
          return allyAccessible[a.category];
        });
        if (allyAttackList.length) {
          ...
          html += '<table class="skills-table" style="font-size:0.82rem;"><thead><tr><th class="skill-name-col">Attack</th><th style="width:5rem;text-align:left;padding-left:0.4rem;">Range</th><th style="text-align:left;padding-left:0.4rem;">Dice Pool</th><th style="text-align:left;padding-left:0.4rem;">Effect</th></tr></thead><tbody>';
          var aLastCat = "";
          allyAttackList.forEach(function(a) {
            if (a.category !== aLastCat) {
              aLastCat = a.category;
              html += '<tr><td colspan="4" style="font-size:0.68rem;text-transform:uppercase;letter-spacing:0.1em;color:var(--uj-amber-light);padding:0.55rem 0.4rem 0.2rem;border-top:1px solid var(--uj-border-cool);">' + esc(aLastCat.charAt(0).toUpperCase() + aLastCat.slice(1).replace(/-/g, " ")) + "</td></tr>";
            }
            var aPool = parseAttackPool(a.attack_dice, allyTraitMap, allySkillFn);
            html += '<tr class="skill-row-active"><td class="skill-name-col" style="font-weight:500;">' + esc(a.name) + '</td><td style="font-size:0.78rem;color:var(--uj-text-dim);padding-left:0.4rem;">' + esc(a.attack_range) + '</td><td style="padding-left:0.4rem;">' + (aPool.length ? '<span style="color:var(--uj-teal);font-weight:600;">' + aPool.join(" + ") + "</span>" : '<span class="skill-die-empty">\u2014</span>') + '</td><td style="font-size:0.78rem;color:var(--uj-text-dim);padding-left:0.4rem;">' + esc(a.effect) + "</td></tr>";
          });
          html += "</tbody></table></div>";
        }
