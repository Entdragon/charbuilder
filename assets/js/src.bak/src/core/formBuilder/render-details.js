...
export default {
  ...
  renderContent(data = {}) {
    // start with a full dice pool and carve out used dice as we go
    let pool = [...DICE];
    ...
    const traitFields = TRAITS.map(trait => {
      ...
      // build options from current pool
      const options = pool
        .map(die => {
      ...
      // once we've rendered this trait's select, remove its value from the pool
      if (val) {
        pool = pool.filter(die => die !== val);
      }
        ...
        <div class="cg-details-panel">
          ...
          <div class="cg-details-box">
            ...
            <input
              type="text"
              id="cg-goal1"
              value="${escape(data.goal1)}"
            />
            <label>Goal 2</label>
            <input
              type="text"
              id="cg-goal2"
              value="${escape(data.goal2)}"
            />
            <label>Goal 3</label>
            <input
              type="text"
              id="cg-goal3"
              value="${escape(data.goal3)}"
            />
          </div>
